defmodule Showme.VideoControllerTest do
  use Showme.ConnCase
  ...
  test "lists all user's videos on index", %{conn: conn, user: user} do
    ...
    other_video = insert_video(insert_user(username: "other guy"), title: "other video")
  ...
  test "creates a user video and redirects", %{conn: conn, user: user} do
    ...
    assert redirected_to(conn, 302) == video_path(conn, :index)
    assert Repo.get_by!(Video, valid_attrs).user_id == user.id
  end

  @tag login_as: "samantha"
